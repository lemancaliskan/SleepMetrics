...
class SemiCircleGauge(ctk.CTkCanvas):
    ...
    def draw_gauge(self, value):
        ...
        self.create_text(cx, cy - 8, text="Uyku Kalitesi (0/10)",
...
class RecommendationBox(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        ctk.CTkLabel(self, text="✨ Size Özel Sağlık Önerileri",
    ...
    def update_recommendations(self, quality, disorder, stress, activity, duration, hr, sys, steps):
        ...
        if "Apne" in disorder:
            recs.append(("Uyku Apnesi", "KBB uzmanına görünün.", "🌬️", "#ff4b4b"))
        elif "İnsomnia" in disorder:
            recs.append(("İnsomnia", "Yatmadan 2 saat önce mavi ışığı kesin.", "📱", "#3498db"))

        if stress > 7:
            recs.append(("Yüksek Stres", "Yatmadan önce nefes egzersizi yapın.", "🧘", "#9b59b6"))

        if duration < 6:
            recs.append(("Düşük Süre", "Vücut onarımı için +7 saat hedefleyin.", "⏰", "#e67e22"))

        if activity < 30:
            recs.append(("Hareketsizlik", "Günlük 20-30 dk yürüyüş derin uykuyu %30 artırır.", "👟", "#2ecc71"))

        if not recs:
            recs.append(("Harika!", "Uyku verileriniz oldukça sağlıklı görünüyor.", "✅", AppColors.SUCCESS))

        if hr > 100:
            recs.append(
                ("Yüksek Nabız", "Kafeini azaltın ve bol su tüketin.", "❤️", "#e74c3c"))

        if sys > 140:
            recs.append(("Yüksek Tansiyon", "Tuzu kısıtlayın, takibi artırın.", "⚠️", "#ff4b4b"))

        if steps < 5000:
            recs.append(("Düşük Adım Sayısı", "Günlük 7500 adım, uykuya dalma süresini %15 kısaltır.", "👣", "#3498db"))
        elif steps < 8000:
            recs.append(("Hareket Hedefi", "Uykuda tam onarım için hedefi 10.000 adıma çıkarın.", "🏃", "#f39c12"))
