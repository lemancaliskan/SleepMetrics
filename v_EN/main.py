...
class SleepApp(ctk.CTk):
    ...
    def setup_ui(self):
        ...
        if logo_img:
            ...
            ctk.CTkLabel(self.sidebar, text="🌙 SLEEPMETRICS",
        ...
        self.btn_analyze = ctk.CTkButton(self.bottom_frame, text="START SLEEP ANALYSIS",
        ...
        self.prediction_label = ctk.CTkLabel(self.info_card, text="Input Data to Analyze...",
    ...
    def setup_form(self):
        fields = [
            ("Sex", "Male", "Gender", ["Female", "Male"]),
            ("Age", 30, "Age", None),
            ("Height (cm)", 175, "Height", None),
            ("Weight (kg)", 75, "Weight", None),
            ("Sleep Duration", 7.5, "Duration", None),
            ("Physical Activity (min)", 60, "Activity", None),
            ("Systolic Blood Pressure", 120, "Systolic", None),
            ("Diastolic Blood Pressure", 80, "Diastolic", None),
            ("Heart Rate (bpm)", 70, "HR", None),
            ("Stress Level (1-10)", 5, "Stress", None),
            ("Daily Step Count", 8000, "Steps", None)
        ]
    ...
    def run_analysis(self):
        try:
            ...
            gender_val = 0 if "Female" in self.inputs["Gender"].input_field.get() else 1
            ...
            quality, disorder = self.engine.predict(input_features, lang="en")

            self.gauge.draw_gauge(quality)
            is_healthy = "Healthy" in disorder

            self.prediction_label.configure(
                text=f"Estimated Sleep Quality: {quality}\n {disorder.upper()}",
            ...
            print(f"Error: {e}")
            self.prediction_label.configure(text=f"Please verify the input!\n({e})", text_color=AppColors.DANGER)
